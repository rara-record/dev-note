...
3. 웹 페이지와 별개로 존재하므로, DOM이나 window 요소에 접근 할 수 없으며,
4. 요청하지 않는 이상, 없는 것이다 다름이 없다.
...
Express.js 서버에서 볼 수 있는 코딩 패턴과 상당히 유사한 방식으로 핸들러를 구현할 수 있다.

1. 먼저, src폴더 안에 `fixtures`폴더와 `mocks`폴더를 만든다.
...
2.  fixtures폴더는 테스트용 데이터를 모아두는 용도이다.
    테스트용 데이터라는 것을 명확히 하기 위해, 따로 폴더를 두어 관리한다.

```typescript
// fixtures/indx.ts
import restaurants from "./restaruats";

export default { restaurants };
```

```typescript
// fixtures/restaruats.ts
...
3.  fixtures폴더에서 mock데이터를 가져온 후, handlers 작성한다.
...
test("App", async () => {
  ...
  await waitFor(() => {
    // waitFor: 콜백함수 안에 있는 것이 될때까지 확인함.
